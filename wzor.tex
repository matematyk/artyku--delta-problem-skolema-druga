...
Na pierwszy rzut oka problem nie jest łatwy do rozstrzygnięcia. Wypisując kolejne wyrazy, możemy wysnuć hipotezę, że owy ciąg również poza pierwszym nie ma wyrazu zerowego. Problem można sformułować w ogólności dla większej ilości równań. Artykuł ten pokaże, że nawet dla prostych ciągów za rozwiązaniem stoi ciekawa matematyka. 
...
\textbf{Różne moduły współczynników $\lambda_1$ i $\lambda_2$}\\
...
Rozważmy kolejny przypadek dla ciągu $u_n$, w którym współczynniki uzyskane z twierdzenia są równe. Podobnie jak poprzednio, możemy jawnie wyrazić wyraz $u_{n}$ jako:
\begin{equation*}
    u_n = a \lambda_1^n + b n \lambda_1^n, 
\end{equation*}
Wykazanie, czy istnieje $n$ takie, że $u_n=0$ staje się proste. A zatem $u_n = 0$, jeśli $(an+b) \lambda_1^n = 0$, co po przekształceniu daje $n = -\frac{a}{b}$.
